...
#print >>sys.stderr, args.bedfile
countfrags = args.countfrags
combinereps = args.combinereps
ensemblgtf = args.ensemblgtf
bamnofeature = args.bamnofeature
trnatable = args.trnatable
trnaaminofile = args.trnaaminofile
...
if combinereps:
    ...
    for currbed in trnalist:
        ...
        if countfrags:
            ...
            print "tRNA_antisense\t"+"\t".join(str(sumsamples(trnaantisense,sampledata,currrep, currbed, sizefactors = sizefactor)) for currrep in replicates)
        else:
            
            print "tRNA\t"+"\t".join(str(sumsamples(trnacounts,sampledata,currrep, currbed, sizefactors = sizefactor)) for currrep in replicates)        
        
    
    for currbed in locilist:
        #print >>sys.stderr, currbed 
        if countfrags:
            print "pretRNA_full\t"+"\t".join(str(sumsamples(fulltrnalocuscounts,sampledata,currrep, currbed, sizefactors = sizefactor)) for currrep in replicates)
            print "pretRNA_partial\t"+"\t".join(str(sumsamples(partialtrnalocuscounts,sampledata,currrep, currbed, sizefactors = sizefactor)) for currrep in replicates)
            print "pretRNA_trailer\t"+"\t".join(str(sumsamples(trnalocustrailercounts,sampledata,currrep, currbed, sizefactors = sizefactor)) for currrep in replicates)
        else:
            print "pretRNA\t"+"\t".join(str(sumsamples(trnalocuscounts,sampledata,currrep, currbed, sizefactors = sizefactor)) for currrep in replicates)
    for currbiotype in emblbiotypes:
        print currbiotype+"\t"+"\t".join(str(sumsamples(emblcounts,sampledata,currrep, currbiotype, sizefactors = sizefactor)) for currrep in replicates)
    for currbed in bedlist:  
        print os.path.basename(currbed)+"\t"+"\t".join(str(sumsamples(counts,sampledata,currrep, currbed, sizefactors = sizefactor)) for currrep in replicates)
    ...
    for currbed in trnalist:
        ...
        if countfrags:
            ...
            print "tRNA_antisense\t"+"\t".join(str(trnaantisense[currsample][currbed]/sizefactor[currsample]) for currsample in samples)
        else:
    ...
    for currbed in locilist:
        if countfrags:
            print os.path.basename(currbed)+"\t"+"\t".join(str(fulltrnalocuscounts[currsample][currbed]/sizefactor[currsample]) for currsample in samples)
            print os.path.basename(currbed)+"\t"+"\t".join(str(partialtrnalocuscounts[currsample][currbed]/sizefactor[currsample]) for currsample in samples)
            print os.path.basename(currbed)+"\t"+"\t".join(str(trnalocustrailercounts[currsample][currbed]/sizefactor[currsample]) for currsample in samples)
        else:
    ...
    for currbiotype in emblbiotypes:
        print currbiotype+"\t"+"\t".join(str(emblcounts[currsample][currbed]/sizefactor[currsample]) for currsample in samples)
    for currbed in bedlist:
...
if args.trnaaminofile is not None:
    trnaaminofile = open(args.trnaaminofile, "w")
    if combinereps:
